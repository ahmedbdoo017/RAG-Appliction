...
class OpenAIProvider(LLMInterface):
    ...
    def __init__(self, api_key:str, api_url: str,
                 ...
                 default_generation_max_output_token:int=100,
    ...
    def generate_text(self, prompt: str,chat_history: list=[], max_output_tokens: int=None , 
        ...
        if not self.generation_model_id:
            self.logger.error("Generation model for openai in wasn't set")
            return None
        ...
        if not response or not response.choices or len(response.choices) == 0 or not response.choices[0].message:
            self.logger.error("error while genaration text with openai")
            return None
    ...
    def embed_text(self, text:str, document_type:str=None):
        ...
        if not self.embedding_model_id:
            self.logger.error("Embbeding model for openai in wasn't set")
            return None
        ...
        if not response or not response.data or len(response.data) == 0 or not response.data[0].embedding:
            self.logger.error("Error while embeding text with openai")
            return None
